...
### Day 8 · Capstone · 课程设计(独立做一个 agent 产品)
- **目标**:综合前 7 天,**独立设计并实现一个 agent 功能产品**。
- **流程**:① 选题(见下)② 写一页设计(要解决什么、该用哪一格 raw/harness/loop/graph、需要哪些工具/MCP、成功标准)③ 实现 ④ 自评(用 Day 7 学的评测思路验收)。
- **选题示例**:仓库问答/改码 agent、文档批处理 agent、带 MCP 的日程/邮件助理、数据分析 agent、网页调研 agent……
- **交付**:一个能跑的 agent 产品 + 一页设计文档 + 一份自评报告。
- **验收判断力**:你能说清"为什么这样设计、为什么用/不用 graph、边界在哪"——而不只是"它能跑"。

## 三道质量护栏(见对应文件)
- [`AGENT.md`](./AGENT.md) — tutor 不替你写答案(练你的判断力)
- [`CONTENT.md`](./CONTENT.md) — 概念对齐主流、API 现查实跑(保证准确可靠)
- [`AUTHORING.md`](./AUTHORING.md) — 章节完整不留洞、高密度、循序渐进(保证干货)
