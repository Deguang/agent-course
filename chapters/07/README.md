...
「orchestrator-workers / 多 agent」听着高级,但 2026 有一场公开辩论**正好把判断力讲透**——两边都对,分歧点极清楚:

- **Anthropic《多 agent 研究系统》(2025-04)**:一个 lead agent 规划,并行开 **3–5 个子 agent(各自独立上下文窗口)**,每个查一条独立线索,最后由 lead 汇总(外加一遍引用核对)。内部评测比单 agent Opus 4 **高 90.2%**——但**代价约是普通对话的 15 倍 token**。适合:**广度优先、路径彼此独立、信息总量超过单个上下文窗口**的研究/检索型任务。
- **Cognition《Don't Build Multi-Agents》**:做**编码**这种**步步依赖、要求连贯**的任务,单 agent 更好——因为多 agent 会**上下文碎片化**,子 agent 各自基于不完整信息做决定,**互相打架**、拼不出一致的产物。
- **连 Anthropic 自己都划线**:"**需要所有 agent 共享同一上下文、或 agent 间依赖很多**的领域,今天不适合多 agent。"

**提炼成可迁移的判断**:多 agent 赢在**读多、可并行、子任务独立、结果能压缩后再回汇**(研究/检索);输在**要共享演进中的上下文、要产出一件连贯的东西**(写代码 / 写长文)。再加上 **~15× 的 token 账**——那份"广度"得真的值这个钱。**这就是本章"别过度上 graph"的量化版**:不是不能上多 agent,而是先问自己两句——"我的子任务真独立吗?它们需要共享上下文吗?"
