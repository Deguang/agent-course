...
### 延伸:Trace 只是起点——Trace → Memory → Skill

trace 记下了"刚才发生了什么",但一次完整 trace **只是一次任务的历史**,信息量大、噪声高。真正能影响未来的,要再往上走两层(把 Day 4 的记忆、Day 6 的 Skill 串起来):

- **Trace = 执行记录**:按时间存工具调用 / 输入输出 / 结果。适合诊断,但只属于**这一次**。
- **Memory = 整理过的经验**:从 trace 里筛出对未来有价值的(用户偏好、环境约束、有效方案、失败原因、关键决策)——就是 **Day 4 的长期 / 经验记忆**。
- **Skill = 可复用的方法**:把**反复验证过**的经验,抽象成"适用条件 + 步骤 + 验收 + 异常处理"——就是 **Day 6 的能力包**。

闭环:**执行 → 产生 Trace → 评估反思 → 沉淀 Memory → 多次验证与抽象 → 形成 Skill → 喂回下一次执行**。

> **判断力(呼应本章 judge 的克制)**:别把一次 trace 直接当记忆,别把一次**偶然成功**直接固化成 skill——**都要先过验证**(正是本章 eval 的用武之地)。这也点出一件事:agent 的"自进化"往往发生在**模型周围的系统(harness)**里,而非模型本身。
...
- **讲回来**([`JOURNAL.md`](../../JOURNAL.md)):为什么"没报错"不等于"做对了"?trajectory eval 比只看最终答案强在哪(举个"答案对但过程错"的例子)?为什么执行崩要记为 fail 而不是让套件抛出?什么时候该用确定性检查、什么时候才动用 LLM-as-judge?LLM judge 有哪些系统偏差,为什么批量跑前必须先拿人工标注校准?Trace、Memory、Skill 三者各是什么、为什么"一次成功"不该直接变成 skill?
