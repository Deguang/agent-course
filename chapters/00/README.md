# Chapter 00 · Day 0 基础:模型、工具、循环的全景
...
```
想:先看看报什么错  →  做:运行测试(工具)  →  看:TypeError
想:是这个文件的问题 →  做:读文件(工具)    →  看:第 12 行错了
...
| 天 | 你会亲手建的 |
|----|------------|
| **1** | **Agent Loop**:model → 工具 → 结果 → model 的循环(含并发/异常/防跑飞) |
| 2 | 接真实模型(消息 IR + 流式 + provider adapter,provider 中立) |
| 3 | 真实工具(读写文件、跑命令,安全边界) |
| 4 | 有状态 & 持久化(跨轮状态 + 可恢复的会话) |
| 5 | 上下文管理(在窗口预算内压缩历史) |
| 6 | 可扩展性(MCP + skills + 扩展隔离) |
| 7 | 编排与多 agent(loop → graph) |
| 8 | 知识层:RAG / 检索(+ 长期记忆) |
| 9 | Guardrails & 人在环路(权限/审批/花费上限/注入防御) |
| 10 | Evals & 可观测(trajectory eval / 回归 / tracing) |
| **11** | **Capstone · 课程设计:独立做一个 agent 产品** |

核心不是"学会调某个 API",而是:**每一块你都先能自己做出来,于是将来 AI 替你写这些时,你看得懂、也纠得动。**

---

## 八、进 Day 1 之前,先想一个(写进 [`LOG.md`](./LOG.md))
...
凭现在的理解答。带着这个猜测进 Day 1,那一章会给你答案,并让你亲手把它建出来。
