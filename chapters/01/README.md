# Chapter 01 · Agent Loop 完整版

> 第 1 天 · 目标:亲手写出一个**完整**的 agent 循环——不只是"能跑一次",而是能处理连续调用、一步多个调用、工具出错、以及防止跑飞。写完,你就掌握了所有 agent 的内核。
...
```
user → model → loop → model → loop → model
        提议A   执行A   提议B   执行B   最终答复
```
...
- **讲回来**(测试全绿后,写进 [`LOG.md`](./LOG.md),讲不清=没真懂):
