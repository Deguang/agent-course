...
```mermaid
flowchart LR
  U["你(用户)"]:::io -->|请求| M[模型]:::model
  M -->|提议调用工具| L[loop 执行工具]:::sub
  L -->|结果喂回,再问一次| M
  M -->|最终答复| A([结束]):::io
```
...
```mermaid
flowchart LR
  U[user]:::io --> M1[model]:::model
  M1 -->|提议A| L1[loop]:::sub
  L1 -->|执行A| M2[model]:::model
  M2 -->|提议B| L2[loop]:::sub
  L2 -->|执行B| M3["model:最终答复"]:::model
```
...
## 三、放到行业里:你手写的这个 loop,别人叫它什么

你建的不是玩具。同一个东西,业界有几套叫法,知道了你就能和文档/同行对上话:

- **ReAct 循环**:`想(reason)→ 做(act,调工具)→ 看(observe,结果)→ 再想`。这就是 2022 年那篇 ReAct 论文的核心,也是几乎所有 agent 的骨架。你写的 while 循环 = ReAct。
- **agent harness(2026 主流叫法)**:把这层"**包住模型、校验/执行/记录每个动作,模型只提议、代码才执行**"的确定性运行时,叫 **harness**。你这章的 `runAgent` 就是**最小 harness**——Day 2 给它补可靠 IO、Day 9 补权限/预算、Day 10 补 trace,都是往这同一个 harness 上叠层。
- **框架里它被藏起来了**:Vercel AI SDK(`generateText({ tools, maxSteps })`)、OpenAI Agents SDK、LangGraph…… 内部跑的就是这个 loop,只是替你写好、包起来了(具体 API 以官方文档为准)。**正因为你亲手写过,才看得懂框架替你做了什么、出问题时改得动**——这正是本课"先从零建,再用框架"的理由。

**一个绕不开的生产现实:loop 每转一圈 = 一次模型往返 = 花钱 + 延迟。** 一个 5 步任务 = 至少 5 次模型调用。所以:

- `maxSteps` 不只防跑飞,**也是成本闸**——它直接封顶"这次任务最多花几次模型调用"。
- 能一次调用解决就别起 loop、能少转就别多转(呼应 Day 0 那句克制:不是越"agent"越好)。
...
- **讲回来**(测试全绿后，写进 [`JOURNAL.md`](../../JOURNAL.md)，讲不清=没真懂):
  ...
  ④ 你手写的 loop、ReAct、"harness" 是不是同一个东西?为什么说 `maxSteps` 也是一道成本闸?
